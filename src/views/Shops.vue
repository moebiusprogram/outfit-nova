<template>
  <section class="tienda">
    <div style="height: 42px;"></div>
      <NavbarSearch :route_to="'/'" :show="true" :white="true" :shadow="showNav === true ? true : false" :rating="true" :visit="true"/>

      <BrandBanner :title="'tienda'" :name="$route.params.shop"/>
      <BrandNav :class="showBrandNav == true ? 'brand-nav-fixed' : ''"
                @updateView="updateView"/>
   
      <div v-if="navTab==='Opiniones'">
      <BrandRatings />
  ...
  un mes y no tengo respuesta." />
      ...
      />
          ...
          <!-- Sub navegacion de tallas/tiendas/precios -->
              <ProductsNav :style="{display: showProfileNav ? none : block}"/>
              <swiper 
              v-if="navTab=='Chollos' || navTab=='Todo' || navTab=='Cupones'"
              :loop="true"
                   :spaceBetween="0"
                   :slidesPerView="1" 
                   :simulateTouch="true"
        ...
        <section class="text-center" v-if="!showProfileNav" style="margin-top:55px; background:#FFF">
        <FooterNavbar  />
    </section> 
  ...
  return {
      ...
      { 
        ...
        tienda_icon:['start','fire','diamante','start','fire','diamante'],
         ...
         slideview:0,
          ...
          name_tabs: ['Nuestra selección', 'Nuevo', 'Top Likes Hoy', 'Tus alertas', 'Más Comentadas', 'Mis favoritos' ],
          name_iconos: ['IconoMedalla','IconoAlerta', 'IconoFire','IconoCampana','IconoCommentPurple','IconoStart'],
    ...
    methods: {
      ...
      handleSlideTo() {
        ...
        ActivartouchSlide()
        {
            return this.secondSwiper.allowTouchMove = true, this.BloquedtouchActiveSlideTallas = true
        },
            getEmitBloquearSwiper(dato, getRatio)
        {
            return this.BloquedtouchActiveSlideTallas = dato, 
    ...
    calculateOffset(scrollPos) 
    {
      ...
      if (scrollPos > maxHeigth + 60) {
        this.showBrandNav = true; //componente con la marca y logo
        this.showProfileNav = true; //barra con "lo más comentado, etc."
