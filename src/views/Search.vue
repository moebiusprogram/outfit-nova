<template>
  <section class="tienda">
    <div style="height: 42px;"></div>
      <NavbarSearch :route_to="'/'" :show="true" :white="true" :name="`Búsqueda ${$route.params.search}`" :shadow="true" :whatsApp="true"/>
      <ProfileNav 
        class='profile-nav-fixed'
        :tabs="name_tabs"
        :onbarra="false"
        :onbackground="true"
      ...
      />
    <nav style="gap:1px; position:sticky;top: 83px; background:white;
    z-index: 10;margin-bottom:35px;">
      <swiper 
        ...
        :initialSlide="1"
        :modules="[Controller]" >
        <swiper-slide v-for="(searchCat, index) in searchCats" :key="index"> 
          <SearchSlide :load="searchCat" :toggled="`style${(index%2)+1}`"></SearchSlide>
        </swiper-slide>
      </swiper>
    </nav>
      ...
      { 
        return {
          ...
          searchCats: [{
            name: 'Categoría',
            chosen: 'Hoodies',
            options: ['Camisetas','Pantalones','Gafas']
          },
          {
            name: 'Subcategorías',
            chosen: 'Camisetas',
